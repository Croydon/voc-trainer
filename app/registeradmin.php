...
if (isset($_POST["username"]) and isset($_POST["password"]) and $_POST["password"] == $_POST["repeat_password"])
{
    $generateUserPasswort = generate_password($_POST["password"], "1");
    $f3->db->exec("INSERT INTO " . $f3->get("prefix") . "user (id, username, pw) VALUE ('1', '" . $f3->clean($_POST["username"]) . "', '" . $generateUserPasswort . "')");
    $f3->reroute("/login");
}
else
{
